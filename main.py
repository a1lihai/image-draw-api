...
class ModelAdapter:
    ...
    def call_jimeng_draw(img_req: UnifiedImageReq) -> List[str]:
        ...
        service_name = "cv"
        region = cfg["region"] if cfg.get("region") else "cn-beijing"
        ...
        resp_data = resp.json()
        logger.info(f"Jimeng Raw Response: {resp_data}")
        meta = resp_data.get("ResponseMetadata", {})
        err_info = meta.get("Error")
        if err_info:
            raise Exception(f"VolcEngine API Error: {err_info['Code']} - {err_info['Message']}")
